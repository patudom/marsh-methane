...
6. [What we checked, and what it proved](#6-what-we-checked-and-what-it-proved)
7. [Honest weaknesses](#7-honest-weaknesses)
8. [Two transcription traps we avoided](#8-two-transcription-traps-we-avoided)
9. [References](#9-references)
10. [Code map and known gaps](#10-code-map-and-known-gaps)

---

## 1. What the interface does

The user chooses a starting global temperature anomaly and presses Start. The app
then steps forward one simulated year at a time from 2026 to 2101, at about eight
years per second.
...
- **A cartoon canister** bubbles methane out of marsh sediment. The number of
  bubbles is directly proportional to the modelled flux: nine bubbles at the
  baseline, so a flux ratio of 1.28 draws twelve. The readout also gives the
  true molecular flux, around 4.3 × 10¹⁵ molecules m⁻² s⁻¹ at the baseline.
- **A line chart** draws atmospheric methane concentration as the years pass,
  against a dashed reference line for the no-extra-warming case.
- **WorldWide Telescope** sits behind everything showing the Earth, centred on
  the marsh at 38.87° N, 76.55° W.

WWT has no separate "show the globe" call. Its render mode is read directly off
the background imageset's type, so switching to an imageset of type `earth` is
what changes the view. The engine builds its own Blue Marble imageset only when
booted with `startMode: "earth"`, which the Vue component never does, so we ship
the imageset ourselves in `public/earth.wtml` and load it at runtime.
...
and applies its temperature response to the whole wetland pool. Section 6 shows
...
`nonWetlandEmission` in the code, and it is verified in section 6.
...
## 6. What we checked, and what it proved
...
| Starting anomaly | Extra warming by 2101 | CH₄ by 2101 | Flux ratio | Bubbles |
| --- | --- | --- | --- | --- |
...
## 8. Two transcription traps we avoided
...
*Nature Geoscience* 14, 225–230, whose median is 0.18 but whose mean is
2.00 ± 1.51.
...
| App shell, layout, the year-by-year loop | `src/MarshMethane.vue` |
| Switching WWT into its Earth view | `src/composables/useEarthView.ts` |
| The Earth imageset WWT loads at runtime | `public/earth.wtml` |
| Thermometer, canister, chart, controls | `src/components/` |
